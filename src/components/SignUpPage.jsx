import React, { useState } from "react";
import { Mail, Lock, User, Eye, EyeOff, Calendar, CheckCircle2, LayoutGrid, Shield } from "lucide-react";
import { useNavigate } from "react-router-dom";
import authApi from "../api/authApi";
import "./SignUpPage.css";
import { startOAuthFlow } from "../utils/Oauthproviders";

const features = [
  { icon: LayoutGrid, text: "Instant leave approvals" },
  { icon: CheckCircle2, text: "Real-time approval & status tracking" },
  { icon: Calendar, text: "Smart calendar with conflict detection" },
  { icon: Shield, text: "Customized leave policies" },
];

const GoogleIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24">
    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
  </svg>
);

const FacebookIcon = () => (
  <svg width="20" height="20" fill="#1877F2" viewBox="0 0 24 24">
    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
  </svg>
);

const GitHubIcon = () => (
  <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24">
    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
  </svg>
);

const LinkedInIcon = () => (
  <svg width="20" height="20" fill="#0A66C2" viewBox="0 0 24 24">
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 23.2 24 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
  </svg>
);

export default function SignUpPage() {
  const [showPassword, setShowPassword] = useState(false);
  const [loading, setLoading] = useState(false);
  const [fieldErrors, setFieldErrors] = useState({});
  const [success, setSuccess] = useState("");
  const navigate = useNavigate();

  const [formData, setFormData] = useState({
    fullName: "",
    email: "",
    password: "",
    confirmPassword: "",
    agreeToTerms: false,
  });

  const handleChange = (e) => {
    const { name, value, type, checked } = e.target;
    setFormData({ ...formData, [name]: type === "checkbox" ? checked : value });
    if (fieldErrors[name]) setFieldErrors((prev) => ({ ...prev, [name]: "" }));
  };

  const validateForm = () => {
    const errors = {};
    if (!formData.fullName.trim()) errors.fullName = "Full name is required";
    if (!formData.email.trim()) errors.email = "Email is required";
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) errors.email = "Invalid email format";
    if (!formData.password) errors.password = "Password is required";
    else if (formData.password.length < 6) errors.password = "Password must be at least 6 characters";
    if (!formData.confirmPassword) errors.confirmPassword = "Please confirm your password";
    else if (formData.password !== formData.confirmPassword) errors.confirmPassword = "Passwords do not match";
    if (!formData.agreeToTerms) errors.agreeToTerms = "Please agree to Terms & Conditions";
    return errors;
  };

  const handleSignUp = async () => {
    if (loading) return;
    setSuccess("");
    const errors = validateForm();
    if (Object.keys(errors).length > 0) {
      setFieldErrors(errors);
      return;
    }
    setFieldErrors({});
    setLoading(true);
    try {
      await authApi.signup({
        fullName: formData.fullName,
        email: formData.email,
        password: formData.password,
        confirmPassword: formData.confirmPassword,
      });
      setSuccess("Account created! Redirecting to login…");
      setTimeout(() => navigate("/login"), 2000);
    } catch (err) {
      const errorData = err.response?.data;
      if (errorData) setFieldErrors({ general: Object.values(errorData)[0] });
      else setFieldErrors({ general: "Signup failed. Please try again." });
    } finally {
      setLoading(false);
    }
  };

  const handleKeyDown = (e) => {
    if (e.key === "Enter") handleSignUp();
  };

  return (
    <div className="signup-root">
      {/* ── Left Panel ── */}
      <div className="signup-left">
        <div className="signup-left-inner">
          <div className="signup-brand">
            <div className="signup-brand-icon">
              <Calendar size={22} color="#fff" />
            </div>
            <span className="signup-brand-name">LeaveOS</span>
          </div>

          <div className="signup-hero">
            <h1 className="signup-hero-title">
              Simplify your<br />
              workplace.<br />
              <span className="signup-hero-accent">Make management effortless.</span>
            </h1>
          </div>

          <ul className="signup-features">
            {features.map(({ icon: Icon, text }) => (
              <li key={text} className="signup-feature-item">
                <span className="signup-feature-icon">
                  <Icon size={16} />
                </span>
                <span>{text}</span>
              </li>
            ))}
          </ul>
        </div>

        {/* Decorative grid overlay */}
        <div className="signup-left-grid" aria-hidden="true" />
      </div>

      {/* ── Right Panel ── */}
      <div className="signup-right">
        <div className="signup-card">
          {/* Logo mark */}
          <div className="signup-card-logo">
            <div className="signup-card-logo-icon">
              <Calendar size={28} color="#fff" />
            </div>
          </div>

          <h2 className="signup-card-title">Create Your Account</h2>
          <p className="signup-card-subtitle">
            Join the Leave System — manage time off with ease
          </p>

          {/* General alert */}
          {fieldErrors.general && (
            <div className="signup-alert signup-alert--error" role="alert">
              <span className="signup-alert-dot" />{fieldErrors.general}
            </div>
          )}
          {success && (
            <div className="signup-alert signup-alert--success" role="alert">
              <CheckCircle2 size={14} />{success}
            </div>
          )}

          {/* Form */}
          <div className="signup-form">

            {/* Full Name */}
            <div className="signup-field-wrapper">
              <div className={`signup-field ${fieldErrors.fullName ? "signup-field--error" : ""}`}>
                <span className="signup-field-icon"><User size={16} /></span>
                <input type="text" name="fullName" value={formData.fullName}
                  onChange={handleChange} onKeyDown={handleKeyDown}
                  placeholder="Full Name" disabled={loading}
                  autoComplete="name" className="signup-input" />
              </div>
              {fieldErrors.fullName && <p className="signup-field-error">▲ {fieldErrors.fullName}</p>}
            </div>

            {/* Email */}
            <div className="signup-field-wrapper">
              <div className={`signup-field ${fieldErrors.email ? "signup-field--error" : ""}`}>
                <span className="signup-field-icon"><Mail size={16} /></span>
                <input type="email" name="email" value={formData.email}
                  onChange={handleChange} onKeyDown={handleKeyDown}
                  placeholder="Your Email" disabled={loading}
                  autoComplete="email" className="signup-input" />
              </div>
              {fieldErrors.email && <p className="signup-field-error">▲ {fieldErrors.email}</p>}
            </div>

            {/* Password */}
            <div className="signup-field-wrapper">
              <div className={`signup-field ${fieldErrors.password ? "signup-field--error" : ""}`}>
                <span className="signup-field-icon"><Lock size={16} /></span>
                <input type={showPassword ? "text" : "password"} name="password"
                  value={formData.password} onChange={handleChange} onKeyDown={handleKeyDown}
                  placeholder="Password" disabled={loading}
                  autoComplete="new-password" className="signup-input signup-input--padded-right" />
                <button type="button" className="signup-eye-btn"
                  onClick={() => setShowPassword((v) => !v)} disabled={loading}
                  aria-label={showPassword ? "Hide password" : "Show password"}>
                  {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
                </button>
              </div>
              {fieldErrors.password && <p className="signup-field-error">▲ {fieldErrors.password}</p>}
            </div>

            {/* Confirm Password */}
            <div className="signup-field-wrapper">
              <div className={`signup-field ${fieldErrors.confirmPassword ? "signup-field--error" : ""}`}>
                <span className="signup-field-icon"><Lock size={16} /></span>
                <input type="password" name="confirmPassword" value={formData.confirmPassword}
                  onChange={handleChange} onKeyDown={handleKeyDown}
                  placeholder="Confirm Password" disabled={loading}
                  autoComplete="new-password" className="signup-input" />
              </div>
              {fieldErrors.confirmPassword && <p className="signup-field-error">▲ {fieldErrors.confirmPassword}</p>}
            </div>

            {/* Terms */}
            <div className="signup-field-wrapper">
              <label className="signup-terms">
                <input type="checkbox" name="agreeToTerms" checked={formData.agreeToTerms}
                  onChange={handleChange} disabled={loading} className="signup-checkbox" />
                <span className="signup-terms-text">
                  I agree to the <a href="#" className="signup-link">Terms &amp; Conditions</a>
                </span>
              </label>
              {fieldErrors.agreeToTerms && <p className="signup-field-error">▲ {fieldErrors.agreeToTerms}</p>}
            </div>

            {/* Submit */}
            <button onClick={handleSignUp} disabled={loading} className="signup-submit-btn">
              {loading ? (<><span className="signup-spinner" />Creating Account…</>) : "Sign up"}
            </button>
          </div>

          {/* Divider */}
          <div className="signup-divider">
            <span className="signup-divider-text">or sign up with</span>
          </div>

          {/* Social Buttons */}
          <div className="signup-socials">
            {[
              { label: "Google",   id: "google",   Icon: GoogleIcon },
              { label: "Facebook", id: "facebook", Icon: FacebookIcon },
              { label: "GitHub",   id: "github",   Icon: GitHubIcon },
              { label: "LinkedIn", id: "linkedin", Icon: LinkedInIcon },
            ].map(({ label, id, Icon }) => (
              <button
                key={label}
                className="signup-social-btn"
                title={`Sign up with ${label}`}
                disabled={loading}
                onClick={() => startOAuthFlow(id)}
              >
                <Icon />
              </button>
            ))}
          </div>

          <p className="signup-login-prompt">
            Already have an account?{" "}
            <button
              className="signup-link signup-link--btn"
              onClick={() => navigate("/login")}
              disabled={loading}
            >
              Sign In
            </button>
          </p>
        </div>
      </div>
    </div>
  );
}